...
public class AssessmentRunner {
    ...
    public static void main(String[] args) {
        ...
        List<CarResult> cleanList = carService.removeDuplicates(CARS);
        List<CarResult> sortedCars = new ArrayList<>();
        
        List<CarResult> corporates = carService.getCorporateCars(cleanList, true);
        for (Character c : carService.getSippRanking()) {
        	sortedCars.addAll(processGroup(corporates, c));
        }
    	sortedCars.addAll(processGroup(corporates, Character.MIN_VALUE));
    	
        List<CarResult> nonCorporates = carService.getCorporateCars(cleanList, true);
        for (Character c : carService.getSippRanking()) {
        	sortedCars.addAll(processGroup(nonCorporates, c));
        }
    	sortedCars.addAll(processGroup(nonCorporates, Character.MIN_VALUE));
        
        display.render(sortedCars);
       
    }        
    
    private static List<CarResult> processGroup(List<CarResult> cars, char sipp) {
    	
    	var carService = CarService.getInstance();
    	
    	List<CarResult> sipps;
    	
    	sipps = carService.getSipp(cars, sipp);
    	carService.sortPrice(sipps);
    	carService.removeAboveMedian(sipps);
    	return sipps;
    }
}